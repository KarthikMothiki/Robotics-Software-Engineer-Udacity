...
void drive_robot(float lin_x, float ang_z)
{
    ...
    if (client.call(srv))
          {
              ROS_INFO("Successfully called the Velocity Service!\n");
          }
          else
          {
...
void process_image_callback(const sensor_msgs::Image img)
{
    ...
    for (int i = 0; i < (img.height * img.step); i+=3) 
    {
        ...
        {
		step_pixel = (i%img.step);
		clear_ball_image = true;
		break;
            
	}
    }

	// If the White Pixel available in the camera frame the condition works
